...
handleEvent event glossWorld
       ...
       -- Left Mouse Button (Down) :: Insert robot onto canvas.
       | (EventKey (MouseButton LeftButton) Down _ (x,y)) <- event,
         GlossWorld (World rs ps) rSpeeds _               <- glossWorld
          = let  newRobot = Robot $ p2 (x,y) ;
                 info     =    "Robots: " ++ (show . succ . length) rs
                            ++ " Packages: " ++ (show . length) ps in
            GlossWorld (World (newRobot:rs) ps) (1.0:rSpeeds) info

       -- Right Mouse Button : Down (Insert package source), Up(Insert package target)
       | (EventKey (MouseButton RightButton) Down _ (x,y)) <- event,
         GlossWorld (World rs ps) rSpeeds _                <- glossWorld
         = let  newPackage = Package (p2 (x,y)) (p2 (x,y)) ;
                info     = "Robots: "   ++ (show . length) rs ++
                           " Packages: " ++ (show . succ . length) ps in
