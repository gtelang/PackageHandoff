...
variants. This section describes the code therein. 

First, import necessary libraries and make some descriptive type-synonyms. 
...
**** Gloss Canvas IO 
Now we will need to connect the world with a Gloss canvas. The following data-structure
will be used by event-handlers which themselves will be wrappers around the algorithmic code. 
...
*Mode* in the snippet above refers to the following type. 
...
The GlossCanvas is always in one of four states(modes) described in the *Mode* data-type above. Depending on the 
current mode of the canvas, it responds differently to mouse and key-press events. To switch between the 4 modes 
use the 'r','f','p','a' keys (or their upper-case brothers). 

Each robot will travel along a piece-wise linear curve, its vertices being source/destinations of a package 
the rendezvous points where the package-handoff occur. And the problems ask us to determine a trajectory for 
each robot so that packages are delivered to their destination in quickest possible time. 

With that in mind, it is natural to represent the schedule for a single robot and a collection of robots as follows. 
